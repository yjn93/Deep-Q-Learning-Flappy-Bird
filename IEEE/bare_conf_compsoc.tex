...
Another issue arose is that the Flappy Bird problem is basically a control problem while the value function alone does not tell the agent how to act without a model. 
\begin{equation}\label{eq:valueModel}
\hat{s} = \hat{\varepsilon} (s,a)
\end{equation}
However, the model can be sometimes very complex in real environments and it usually suffers from very poor accuracy. \cite{strehl2006pac}
